...
void updateTree (node* past, node* present, int start, int finish, int index, int key)
{
    ...
    if ( index <= centre)
    {
        ...
        accessNode(present -> left, NULL, NULL, 0);
    ...
    else 
    {
        ...
        accessNode (present -> right, NULL, NULL, 0);
...
{
    if ( begin > finish || end < finish || finish > start)
    {
        return false;
    }
...
{   
    ...
    //{   
        ...
        {   
            ...
            accessNode( version[i], NULL, NULL, 0 );
